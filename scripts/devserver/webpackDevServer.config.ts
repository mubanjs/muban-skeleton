...
export function createDevServerConfig(proxy, allowedHost) {
  return {
    ...
    contentBase: [paths.publicAssetPath, paths.pagesAssetPath],
    contentBasePublicPath: [paths.publicPath, paths.publicPath + 'static/'],
    ...
    watchOptions: {
      ignored: ignoredFiles(paths.srcPath),
    },
    ...
    after(app) {
      ...
      app.use('/', async (req, res) => {
        ...
        // served statically
        const asset = getAsset(`.${req.path}`);
        if (asset) {
          res.send(asset);
          return;
        }

        // respond with 404 for any files that have an extension and were not
        // already resolved (with the exception of .html files, we will handle
